...
export const uploadResume = async (token: string, metadata: object, file: File) => {
...
export const markResumePublic = async (token: string, resumeId: string) => {
  const response = await apiClient.put(`/mark-resume-public/${resumeId}`, null, {
    headers: { Authorization: `Bearer ${token}` },
  });
  return response.data;
};

export const generateCoverLetter = async (token: string, resumeId: string, jobDesc: string) => {
